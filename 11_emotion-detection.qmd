...
Emotion detection is another NLP technique aimed at identifying and quantifying human emotions expressed in text, which builds directly on traditional sentiment polarity analysis focusing on capturing more nuanced emotional states.
...
The NRC Emotion Lexicon was developed as part of research into affective computing and sentiment analysis using a combination of manual annotation and crowdsourcing. Human annotators from Mechanical Turk evaluated thousands of words, indicating which emotions were commonly associated with each word. This method ensured that the lexicon captured human-perceived emotional associations, rather than relying solely on statistical co-occurrences in text.
...
```
Now that we have a better understandig of this package, let's get back to business and perform emotion detection to our data. First, let's compute the sentiment score by sentence:
...
Open the `emotion_score` data frame. Notice that we have counts, per emotion, it is not only binary, that is because it computes the number of words associated with a particular emotion within the same string of text.

![](images/emotion-scores-01.png)

We can now generate a summary output for each emotion by running:

``` r
summary(emotion_score)
```

![](images/summary_emotionscores.png)

Let's now bind the two dataframes column-wise, so each row in preprocessed now has the corresponding emotion metrics appended as new columns and linked to the corresponding id:
...
And then, let's sort emotions by frequency and ensures that the factor levels of emotion match this order, which is especially handy for creating plots that reflect the sorted counts.
...
What about plotting it by season?
...
In case you are curious, we can also check which emotions frequently occur together in our dataset:
...
We could continue exploring the data in many ways, especially if we had demographics of users associated with their comments, but that’s beyond the scope of this workshop.

Wait, don't forget to save the emotion results:
...
Next, we will wrap it up with some final considerations.

::: callout-note
## Why using `sentimentr` then?
