...
`text_expand` using the package `textclean`.
...
For steps 3-6 we will be using use the packages `dplyr` and `stringr`
from `tidyverse`.
...
Alright, time to remove punctuations, numbers and repeated letters used
...
Alright, so we need to call the `emoji` package to assist us with this
process.
...
Have you checked out the emoji dictionary we loaded into our RStudio
environment? You might be surprised by how many emojis are included—and
the meaning of some of them!

Now that we’ve handled emojis, we’re ready to move on to the next
